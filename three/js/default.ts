...
module M {
  ...
  app.onactivated = args => {
    if (args.detail.kind === activation.ActivationKind.launch) {
      ...
      (<any>args).setPromise(WinJS.UI.processAll());
    }
  };


  // Allocate space for joint locations
  var createJointPoints = () => {
    var jointPoints = [];
  ...
  var capturedObject: THREE.Mesh;

  // Handles the body frame data arriving from the sensor
  function reader_BodyFrameArrived(args: any) {
    // get body frame
        ...
        if (body.isTracked) {
          ...
          if (bodyImageProcessor.processJointLocations(joints, jointPoints)) {
            ...
            var xr = joints.lookup("11").position.x,
              yr = -1 * joints.lookup("11").position.y,
              zr = (-1 * joints.lookup("11").position.z) + (1.8 * groupCenter.z);

            var xl = 0.4 + joints.lookup("7").position.x,
              yl = 0 + joints.lookup("7").position.y,
  ...
  function sensor_IsAvailableChanged(args: any) {
